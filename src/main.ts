...
async function main() {
  ...
  const cards = parseCells(fileContents, ",")
    .filter(
      // Select all campaign cards, filter out stat cards
      (row) =>
        row[Column.DeckCategory].toLowerCase().trim() === "campaign" && row[Column.Type].toLowerCase().trim() !== "stat"
    )
    .map((row) => ({
  ...
  await writeFile("./cards.yaml", stringify({ cards }));
}
